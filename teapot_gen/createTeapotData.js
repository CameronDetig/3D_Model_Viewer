...
// returns vertices and normals for a particular resolution
    ...
    // Create a deep copy of vertices to avoid mutating the original array
    var workingVertices = teapotVertices.map(v => [...v]);

    for(var i = 0; i<306; i++) for (j=0; j<3; j++)
      sum[j] += workingVertices[i][j];
      for(j=0; j<3; j++) sum[j]/=306;
        for(var i = 0; i<306; i++) for(j=0; j<2; j++)
       workingVertices[i][j] -= sum[j]/2
           for(var i = 0; i<306; i++) for(j=0; j<3; j++)
       workingVertices[i][j] *= 2;
    ...
    for (var i=0; i<numTeapotPatches; i++) 
        for (j=0; j<16; j++) {
            patch[i][j] = vec4([workingVertices[teapotIndices[i][j]][0],
             workingVertices[teapotIndices[i][j]][2], 
                workingVertices[teapotIndices[i][j]][1], 1.0]);
    }
    ...
    for(var i=0; i<=numDivisions; i++) for(var j=0; j<= numDivisions; j++) { 
        ...
        for(var ii=0; ii<4; ii++) for(var jj=0; jj<4; jj++) {
            ...
            data[i][j] = add_legacy(data[i][j], temp);
        }
    }
    ...
    for(var i=0; i<=numDivisions; i++) for(var j=0; j<= numDivisions; j++) { 
        ...
        for(var ii=0; ii<4; ii++) for(var jj=0; jj<4; jj++) {
            ...
            tdata[i][j] = add_legacy(tdata[i][j], temp);
            
            var stemp = vec4(patch[n][4*ii+jj]); ;            
            stemp = scale( ss[ii][jj], stemp);
            sdata[i][j] = add_legacy(sdata[i][j], stemp);

        }
        temp = cross_legacy(tdata[i][j], sdata[i][j])
